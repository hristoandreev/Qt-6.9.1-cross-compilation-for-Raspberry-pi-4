# Qt 6.9.1 cross compilation for Raspberry pi 4
...
libatspi2.0-dev libgl1-mesa-dev libglu1-mesa-dev freeglut3-dev libgmp-dev libmpfr-dev \
libmpc-dev texinfo gawk libdrm-dev open-vm-tools-desktop
```
...
cd host-build/
cmake ../../qt6/ -GNinja -DCMAKE_BUILD_TYPE=Release -DQT_BUILD_EXAMPLES=OFF -DQT_BUILD_TESTS=OFF -DCMAKE_INSTALL_PREFIX=$HOME/qt6/host
...
rsync -avz --rsync-path="sudo rsync" wolfberry@192.168.0.227:/usr/include rpi-sysroot/usr
rsync -avz --rsync-path="sudo rsync" wolfberry@192.168.0.227:/lib rpi-sysroot
rsync -avz --rsync-path="sudo rsync" wolfberry@192.168.0.227:/usr/lib rpi-sysroot/usr 
rsync -avz --rsync-path="sudo rsync" wolfberry@192.168.0.227:/opt/vc rpi-sysroot/opt
```
...
set(TARGET_SYSROOT /home/hristo/rpi-sysroot) ## update this path accordingly
...
```bash
rsync -avz --rsync-path="sudo rsync" $HOME/qt6/pi/* wolfberry@192.168.0.227:/usr/local/qt6
```
